...
export class ChoiceScene extends Phaser.Scene {
    ...
    addCookie(cookieSize, posX) {
        ...
        if (this.cookiePool.getLength()) {
            ...
            cookie = this.physics.add.sprite(posX, +this.game.config.height / 2, "cookie");
    ...
    update() {
        ...
        if (minDistance > this.nextPlatformDistance) {
            var nextCookieWidth = Phaser.Math.Between(gameOptions.platformSizeRange[0], gameOptions.platformSizeRange[1]);
            this.addCookie(+nextCookieWidth, +this.game.config.width + +this.game.config.width / 2);
        }
